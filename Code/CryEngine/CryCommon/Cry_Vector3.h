...
template <typename F> struct __passinreg Vec3_tpl
{
  ...
  ILINE Vec3_tpl( const Vec3_tpl& v ) : x(v.x), y(v.y), z(v.z)  { /*assert(this->IsValid());*/ }
#endif 
...
#if !defined(__SPU__) && defined(PS3) || defined(XENON)
	ILINE Vec3_tpl &operator=(const Vec3_tpl &src) { x=src.x; y=src.y; z=src.z; return *this; }
#endif
