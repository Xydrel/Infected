...
namespace stl
{
	...
	{
		OPT_STRUCT(FHeap)
		VAR_OPT(size_t,	PageSize);			// Pages allocated at this size, or multiple thereof if needed.
	...
	public:
		...
		HeapAllocator(FHeap opts = 0)
			...
			_pPageList(0)
		{
			_PageSize = max( Align(_PageSize, DefaultPageSize), (size_t)DefaultPageSize );
		}
		...
		void* Allocate(const Lock& lock, size_t nSize, size_t nAlign = DefaultAlignment)
		{
			for (;;)
			{
				...
				nAllocSize = RoundUpTo(nAllocSize, _PageSize);
